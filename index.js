...
import {
  sendImageAsSticker2,
  sendVideoAsSticker2,
} from './lib/whatsapp/sticker/sticker2.js';
...
const welcome = JSON.parse(
	fs.readFileSync('./settings/Grupo/Json/welcome.json'),
);
const antilink = JSON.parse(
	fs.readFileSync('./settings/Grupo/Json/antilink.json'),
);
...
const registro = JSON.parse(
	fs.readFileSync('./settings/Grupo/Json/registros.json'),
);
...
async function main() {
	...
	if (!sock.authState.creds.registered) {
		...
		try {
			...
			console.error(
				chalk.red('❌ Error al generar código de vinculación:'),
				err.message,
			);
			process.exit(1);
		}
	...
	sock.ev.on(
		...
		createGroupHandler(sock, { welcome }),
	);
	...
	sock.ev.on('messages.upsert', async (m) => {
		...
		try {
			...
			const commandArgs = hasPrefix
				? lowerMessage
						.slice(
							prefixes.find((prefix) => lowerMessage.startsWith(prefix)).length,
						)
						.trim()
						?.split(' ')
				: [];
			...
			function guardarEstadoBot(estado) {
				fs.writeFileSync(
					estadoPath,
					JSON.stringify({ activo: estado }, null, 2),
				);
				botActivo = estado;
			}
			...
			const modoAdminList = fs.existsSync(modoAdminPath)
			...
			const { getQuotedTypes } = await import('./lib/core/types.js');
			...
			switch (comando) {
				...
				case 'bienvenida':
					if (!isGroup) return;
					if (args.length < 1)
						return enviar('「 𝟏 𝐏𝐚𝐫𝐚 𝐀𝐜𝐭𝐢𝐯𝐚𝐫 𝐲 𝟎 𝐏𝐚𝐫𝐚 𝐃𝐞𝐬𝐚𝐜𝐭𝐢𝐯𝐚𝐫 」 ');
					if (!isGroupAdmins) return enviar(respuesta.admin);
					if (!isBotGroupAdmins)
						return enviar('El bot necesita ser administrador');
					if (Number(args[0]) === 1) {
						if (isWelcome)
							return enviar(
								'「 ✅ 𝐋𝐚 𝐁𝐢𝐞𝐧𝐯𝐞𝐧𝐢𝐝𝐚 𝐘𝐚 𝐄𝐬𝐭𝐚 𝐀𝐜𝐭𝐢𝐯𝐚𝐝𝐚 𝐄𝐧 𝐄𝐥 𝐆𝐫𝐮𝐩𝐨 」 ',
							);
						welcome.push(from);
						fs.writeFileSync(
						...
						if (!isWelcome)
							return enviar('「 ❌ 𝐋𝐚 𝐛𝐢𝐞𝐧𝐯𝐞𝐧𝐢𝐝𝐚 𝐧𝐨 𝐞𝐬𝐭𝐚 𝐚𝐜𝐭𝐢𝐯𝐚𝐝𝐚」');
				...
				case 'revivir':
					...
					if (!isGroup)
						return enviar(
							'Es enserio invocar en un chat , te violo tu tio verdad',
						);
					if (!isGroupAdmins) return enviar(respuesta.admin);
					members_id = [];
...
\n`;
				...
				case 'modoadmin':
					{
						if (!isGroup)
							return enviar('⚠️ Este comando solo se puede usar en grupos.');
						if (!isGroupAdmins)
							return enviar(
								'🚫 Solo los administradores pueden cambiar este modo.',
							);
						...
						if (estado === '1') {
							if (!modoAdmin.includes(from)) {
								...
								enviar(
									'✅ *Modo admin activado* — Ahora solo los administradores pueden usar el bot en este grupo.',
								);
							} else {
							...
							if (modoAdmin.includes(from)) {
								...
								enviar(
									'🟢 *Modo admin desactivado* — Todos los miembros pueden usar el bot nuevamente.',
								);
							} else {
							...
							enviar(
								'❌ Solo puedes usar *1* para activar o *0* para desactivar.',
							);
						}
					}
					break;
				...
				case 'notify':
					{
						...
						if (!isGroupAdmins) return enviar(respuesta.admin);
						if (!q)
							return enviar('Digite un texto ejemplo !notify hola hermanos 🔥');
						if (!isGroup) return enviartexto('Enserio , hidetag en un chat');
						if (!isGroupAdmins)
							return enviartexto('El bot necesita ser administrador');
				...
				case 'largate':
					{
						...
						if (!mentioned)
							return enviar(
								'⚠️ Debes mencionar a alguien para usar este comando.',
							);
				...
				case 'antilink':
					...
					if (!isBotGroupAdmins) return enviar(respuesta.botadmin);
					if (args.length < 1)
						return enviar(`𝐃𝐈𝐆𝐈𝐓𝐄 𝟏 𝐏𝐀𝐑𝐀 𝐀𝐂𝐓𝐈𝐕𝐀𝐑 𝐘 𝟎 𝐏𝐀𝐑𝐀 𝐃𝐄𝐒𝐀𝐂𝐓𝐈𝐕𝐀𝐑`);

					if (Number(args[0]) === 1) {
						if (isAntiLink)
							return enviar('✅ El antilink ya está activado en este grupo');
						antilink.push(from);
						fs.writeFileSync(
							'./settings/Grupo/Json/antilink.json',
							JSON.stringify(antilink, null, 2),
						);
						enviar('✅ ● ️𝐀𝐍𝐓𝐈𝐋𝐈𝐍𝐊 𝐀𝐂𝐓𝐈𝐕𝐀𝐃𝐎 ● ️');
					} else if (Number(args[0]) === 0) {
						if (!isAntiLink)
							return enviar('❌ El antilink ya está desactivado en este grupo');
...
${groupName} `;
							...
							const encmediaa = await sendImageAsSticker2(
								sock,
								from,
								owgi,
								info,
...
${groupName} `;
							...
							const encmedia = await sendVideoAsSticker2(
								sock,
								from,
								owgi,
								info,
				...
				case 'ship':
					{
						...
						if (
							...
							info.message.extendedTextMessage.contextInfo.mentionedJid.length <
								2
						)
							return enviar(
								'💞 Menciona a dos personas para calcular su *nivel de amor* 💘',
							);
						...
						try {
							...
							foto1 =
								'https://i.postimg.cc/VkqDjMdJ/75499-coraz-el-dia-de-san-valentin-amor-corazon-de-amor-x750.jpg';
						}
...
👉 *!emojimix 😊+😂*`);
				...
				case 'tomp3':
					if (!isReg) return enviar(respuesta.registro);
					if (!isQuotedVideo)
						return enviar(`[❗] ${sender?.split('@')[0]}, Marque un video `);
					enviar('`Creando....`');
					...
					try {
						...
						sock
							.sendMessage(
								...
								{
									image: buff,
									caption: ` [❗] *${pushname}*, Aquí tienes tu pedido `,
								},
								{ quoted: info },
							)
							.catch((e) => {
								console.log(e);
								enviar(
									'Nose pudo convertir a imagen verifica que sea un sticker y no un gif ❌',
								);
							});
				...
				case 'ytsearch':
					{
						if (!q)
							return enviar('❌ Escribe un nombre para buscar en YouTube');
						try {
							...
							if (!apiData?.status)
								return reply(
									'❌ No se pudo obtener respuesta válida de la API',
								);
...
`;
				...
				case 'tragamoneda': {
					...
					if (coins < apuestas)
						return enviar('No tienes suficientes Coins 🪙 para jugar.');
					...
					if (tiempoRestante > 0) {
						return await enviar(
							`[❗] 𝙴𝚂𝙿𝙴𝚁𝙴 ${runtime(tiempoRestante / 1000)} para volver a jugar.`,
						);
					} else {
					...
					let resultadoMensaje =
						'😢 Has perdido... Inténtalo de nuevo dentro de 8 horas.';
					let premioTexto = '';
				...
				case 'daily': {
					...
					if (dayli) {
						...
						return sock.sendMessage(
							from,
							{ text: `Espere ${runtime(resultado)} para su nueva recompensa` },
							{ quoted: info },
						);
					} else {
...
`);
				...
				case 'ruleta':
					{
						if (!q)
							return enviar(`Indique un monto para apostar, ejemplo .ruleta 4`);
						if (!isReg) return enviar(respuesta.registro);
						const montto = q;
						const monto = (montto * 1) / 1;
						if (Number.isNaN(monto))
							return enviar(`Indique un monto válido en coins`);
						if (monto > economy.MoneyOfSender(sender))
							return enviar(`No tienes suficiente dinero`);
						if (monto > 5)
							return enviar('La apuesta no debe ser mayor a 5 Rupias');
...
`;
				...
				case 'enviar':
					{
						if (!isGroup)
							return enviar('⚠️ Este comando solo funciona en grupos.');
						...
						(async () => {
							try {
								...
								if (!mencionado)
									return enviar(
										'⚠️ Debes mencionar a alguien para enviarle monedas.\nEj: .regalar @usuario 100',
									);
								if (mencionado === emisor)
									return enviar('⚠️ No puedes enviarte monedas a ti mismo.');
								if (Number.isNaN(monto) || monto <= 0)
									return enviar(
										'⚠️ Ingresa una cantidad válida de monedas.\nEj: .regalar @usuario 100',
									);

								const saldoEmisor = await economy.MoneyOfM(emisor);
								if (saldoEmisor < monto)
									return enviar(
										'❌ No tienes suficientes monedas para hacer esta transferencia.',
									);
...
`;
					if (myrep < 20) {
						await sock.sendMessage(
							...
							{
								image: {
									url: 'https://i.postimg.cc/NfJfvsBW/Untitled-05-12-2024-09-16-50-1.png',
								},
						...
						await sock.sendMessage(
							...
							{
								image: {
									url: 'https://i.postimg.cc/PxjdQNQ8/Untitled-05-12-2024-09-16-50-2.png',
								},
								caption: mitulamide30milimetros,
							},
							{ quoted: info },
						...
						await sock.sendMessage(
							...
							{
								image: {
									url: 'https://i.postimg.cc/HL5pMbXg/Untitled-05-12-2024-09-16-50-3.png',
								},
								caption: mitulamide30milimetros,
							},
							{ quoted: info },
						...
						await sock.sendMessage(
							...
							{
								image: {
									url: 'https://i.postimg.cc/brWX3NWB/IMG-20241223-WA0014.jpg',
								},
								caption: mitulamide30milimetros,
							},
							{ quoted: info },
						...
						await sock.sendMessage(
							...
							{
								image: {
									url: 'https://i.postimg.cc/Cx9hdcZ7/Untitled-05-12-2024-09-16-50-5.png',
								},
								caption: mitulamide30milimetros,
							},
							{ quoted: info },
...
*༻❦ 𝐑𝐀𝐍𝐊𝐈𝐍𝐆 𝐃𝐄 𝐑𝐄𝐏𝐔𝐓𝐀𝐂𝐈𝐎𝐍 ❦༺\n`;
					registro
						.sort((a, b) => b.rep - a.rep) // Ordena de mayor a menor
...
`);
					...
					if (q.startsWith('1')) {
						if (coins < 50)
							return enviar(
								'❌ No tienes suficientes Reales para hacer esta compra. Necesitas al menos 50 Rupias.',
							);
						await economy.delkoin(sender, 50);
						await economy.addXp(sender, 200);

						return enviar(
							`🐱💬 Gracias ${pushname}, cambiaste 50 Rupias por 200 EXP.`,
						);
					}

					if (q.startsWith('2')) {
						const args = q?.split(' ');
						const nivel = parseInt(args[1], 10);
						const nuevoNombre = args.slice(2).join(' ');

						if (Number.isNaN(nivel) || !nuevoNombre) {
							return enviar(
								'❌ Usa el comando correctamente: .tienda 2 <nivel> <nuevo nombre>\nEjemplo: .tienda 2 8 Los Poderosos',
							);
						}

						if (coins < 50) {
							return enviar(
								'❌ No tienes suficientes Coins para cambiar el nombre del rango. Necesitas 50 Coins.',
							);
						}
						...
						try {
							...
							return enviar(
								'⚠️ Error al leer los rangos. Asegúrate de que el archivo exista y esté bien formado.',
							);
						}
						...
						try {
							...
							return enviar(
								'⚠️ No se pudo guardar el nuevo nombre. Intenta de nuevo.',
							);
						}
					}
						...
						try {
							...
							if (apiData && apiData.Estado === 'online') {
								...
								await sock.sendMessage(
									...
									{
										video: {
											url: `${data.EnlaceDescarga}&apikey=${NAUFRA_KEY}`,
										}, // 🔑 Agregar key también aquí
										mimetype: 'video/mp4',
										caption: data.Titulo,
									},
									{ quoted: info },
