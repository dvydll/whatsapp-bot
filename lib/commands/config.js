...
module.exports = {
  ...
  botoff: (params) => {
    ...
    enviar('😴 El bot ha sido *DESACTIVADO* y afectará de responder a los comandos.')
  },
  ...
  antiprivado: (params) => {
    ...
    if (args[0] === 'on') {
      if (Antipv.includes('activo')) return enviar('El anti-privado ya esta activo')
      Antipv.push('activo')
      fs.writeFileSync(chatPath, JSON.stringify(Antipv))
      enviar('Anti-privado activado exitosamente')
    } else if (args[0] === 'off') {
      if (!Antipv.includes('activo')) return enviar('El anti-privado ya estaba desactivado')
  ...
  rvisu: (params) => {
    ...
    try {
      if (JSON.stringify(info).includes("videoMessage")) {
        const vio = info.message?.extendedTextMessage?.contextInfo?.quotedMessage
        const viewVideo = vio?.videoMessage || info.message?.videoMessage || 
                         vio?.viewOnceMessageV2?.message?.videoMessage || info.message?.viewOnceMessageV2?.message?.videoMessage ||
                         vio?.viewOnceMessage?.message?.videoMessage || info.message?.viewOnceMessage?.message?.videoMessage
        ...
        const vio = info.message?.extendedTextMessage?.contextInfo?.quotedMessage
        const viewImage = vio?.imageMessage || info.message?.imageMessage || 
                         vio?.viewOnceMessageV2?.message?.imageMessage || info.message?.viewOnceMessageV2?.message?.imageMessage ||
                         vio?.viewOnceMessage?.message?.imageMessage || info.message?.viewOnceMessage?.message?.imageMessage
