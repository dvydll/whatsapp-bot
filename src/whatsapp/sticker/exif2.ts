...
async function imageToWebp2(media: Buffer): Promise<Buffer> {
...
async function videoToWebp2(media: Buffer): Promise<Buffer> {
  const tmpFileOut = path.join(
    tmpdir(),
    `${Crypto.randomBytes(6).readUIntLE(0, 6).toString(36)}.webp`,
  );
...
async function writeExifImg2(media: Buffer, metadata: StickerMetadata): Promise<Buffer | undefined> {
  const wMedia = await imageToWebp2(media);
  const tmpFileIn = path.join(
    tmpdir(),
    `${Crypto.randomBytes(6).readUIntLE(0, 6).toString(36)}.webp`,
  );
...
async function writeExifVid2(media: Buffer, metadata: StickerMetadata): Promise<Buffer | undefined> {
  const wMedia = await videoToWebp2(media);
  const tmpFileIn = path.join(
    tmpdir(),
    `${Crypto.randomBytes(6).readUIntLE(0, 6).toString(36)}.webp`,
  );
...
async function writeExif2(media: { mimetype?: string; data?: Buffer }, metadata: StickerMetadata): Promise<Buffer | string> {
  const wMedia = /webp/.test(media.mimetype ?? '')
    ? media.data ?? Buffer.alloc(0)
    : /image/.test(media.mimetype ?? '')
      ? await imageToWebp2(media.data ?? Buffer.alloc(0))
      : /video/.test(media.mimetype ?? '')
        ? await videoToWebp2(media.data ?? Buffer.alloc(0))
        : Buffer.alloc(0);
  const tmpFileIn = path.join(
    tmpdir(),
    `${Crypto.randomBytes(6).readUIntLE(0, 6).toString(36)}.webp`,
  ...
  return wMedia as Buffer;
}

export { imageToWebp2, videoToWebp2, writeExif2, writeExifImg2, writeExifVid2 };